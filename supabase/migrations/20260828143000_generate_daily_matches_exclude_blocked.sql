-- Exclude blocked pairs from daily match generation. blocked_users
-- (20260828140000_create_blocked_users.sql) is the source of truth for
-- user-initiated blocks and must persist even after a match row is deleted,
-- so generate_daily_matches() needs its own explicit exclusion rather than
-- relying on the matches-status check alone.
--
-- Scope: adds a blocked_users exclusion to the candidate query only.
-- Scoring, daily cap, cron cadence, distance/country filtering,
-- insert/exception handling all unchanged from
-- 20260822120000_304df65c-92c1-44e8-a05b-05eacafbd5e1.sql.
  ...
  LOOP
    ...
    FOR potential_match IN
      ...
      WHERE uo2.user_id != user_record.user_id
        ...
        -- Exclude any pair blocked in either direction (blocked_users,
        -- added in 20260828140000_create_blocked_users.sql) so blocked
        -- users are never suggested to each other again.
        AND NOT EXISTS (
