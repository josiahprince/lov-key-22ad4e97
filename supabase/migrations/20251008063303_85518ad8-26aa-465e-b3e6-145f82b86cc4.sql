-- Update the generate_daily_matches function to create 2 matches per day instead of 3
...
BEGIN
  ...
  LOOP
    ...
    -- Find potential matches for this user (TESTING: Allow unlimited re-matching)
    FOR potential_match IN
      ...
      WHERE uo2.user_id != user_record.user_id
        ...
        -- TESTING: Only exclude current active matches to allow re-matching
        AND NOT EXISTS (
