-- Update generate_daily_matches function to handle missing location data
  ...
  LOOP
    ...
    FOR potential_match IN
      ...
      WHERE uo2.user_id != user_record.user_id
        ...
        -- Don't match users who already have a match/chat between them
        AND NOT EXISTS (
    ...
    LOOP
      ...
      IF user_record.latitude IS NOT NULL AND user_record.longitude IS NOT NULL 
        ...
        SELECT 6371 * acos(
          cos(radians(user_record.latitude)) * 
      ...
      ELSE
        ...
        IF user_record.country != potential_match.country THEN
          CONTINUE;
        END IF;
        -- Optional: more restrictive location matching for users without coordinates
        -- IF user_record.region != potential_match.region THEN
        --   CONTINUE;
        -- END IF;
      END IF;
      ...
      IF match_score_calc >= 20 THEN
        ...
        INSERT INTO public.matches (
          user_1, user_2, matched_on, expires_at, compatibility_score, status, chat_request_status
        ) VALUES (
          user_record.user_id, 
