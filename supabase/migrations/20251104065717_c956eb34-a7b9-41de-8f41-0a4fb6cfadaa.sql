-- Fix onboarding shown multiple times and duplicate matches issues

-- 1. Add unique constraint on user_onboarding.user_id to prevent duplicate records
-- First, clean up any duplicate onboarding records (keep the most recent one)
...
-- 2. Add unique constraint on matches to prevent duplicate matches
-- First, clean up any existing duplicate matches (keep the most recent one)
DELETE FROM matches
WHERE id NOT IN (
  SELECT DISTINCT ON (LEAST(user_1, user_2), GREATEST(user_1, user_2)) id
  FROM matches
  ORDER BY LEAST(user_1, user_2), GREATEST(user_1, user_2), created_at DESC
);

-- Add unique constraint to prevent the same pair of users from being matched multiple times
-- This constraint ensures that user_1 and user_2 combination is unique regardless of order
CREATE UNIQUE INDEX matches_unique_pair_idx ON matches (LEAST(user_1, user_2), GREATEST(user_1, user_2))
WHERE status IN ('active', 'chatting');

-- 3. Update generate_daily_matches function to better handle duplicate prevention
  ...
  LOOP
    ...
    FOR potential_match IN
      ...
      WHERE uo2.user_id != user_record.user_id
        ...
        -- IMPROVED: Exclude any existing match between these users (both directions, any status)
        AND NOT EXISTS (
    ...
    LOOP
      ...
      IF match_score_calc >= 20 THEN
        ...
        -- Skip if potential match already has 2 matches today
        IF user_matches_today >= 2 THEN
          CONTINUE;
        END IF;
        
        -- Create the match (always insert with smaller user_id as user_1)
        BEGIN
          ...
          -- Re-check this user's matches today count
          SELECT COUNT(*) INTO user_matches_today
          FROM public.matches 
          WHERE (user_1 = user_record.user_id OR user_2 = user_record.user_id)
            AND matched_on >= today_start 
          ...
          -- Stop if this user now has 2 matches for today
          IF user_matches_today >= 2 THEN
            EXIT;
          END IF;
