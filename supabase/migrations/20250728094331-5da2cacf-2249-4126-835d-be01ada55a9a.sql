-- Fix the generate_daily_matches function to prevent duplicate matches and ensure proper exclusion logic
...
BEGIN
  ...
  LOOP
    ...
    FOR potential_match IN
      ...
      WHERE uo2.user_id != user_record.user_id  -- Point 4: Don't match with themselves
        ...
        -- Point 1: Gender/Sexual Orientation/Interested In compatibility checks
        AND (
        ...
        -- Point 2: Age preferences (part of location/compatibility matching)
      ...
      ORDER BY 
        -- Point 2: Location proximity prioritization
    ...
    LOOP
      -- Point 3: Check if active match already exists (allow re-matching only with non-active matches)
      SELECT COUNT(*) INTO existing_match_count
      FROM public.matches m 
      WHERE ((m.user_1 = user_record.user_id AND m.user_2 = potential_match.user_id)
         OR (m.user_1 = potential_match.user_id AND m.user_2 = user_record.user_id))
        AND m.status = 'active';
        
      -- Skip if active match already exists
      ...
      -- Point 2: Location score calculation
      ...
      -- Insert the match with expiry (with conflict handling)
      BEGIN
