-- Fix the generate_daily_matches function to resolve the ORDER BY issue
    ...
    FOR potential_match IN
      WITH compatible_users AS (
        SELECT uo2.user_id, uo2.mood, uo2.selected_memes, pr2.city, pr2.region, pr2.country,
               ...
               -- Calculate distance and compatibility scores for ordering
               CASE 
               ...
               CASE 
                 ...
                 ELSE 0
               END as mood_score,
               array_length(array(SELECT unnest(user_record.selected_memes) INTERSECT SELECT unnest(uo2.selected_memes)), 1) as shared_memes
      ...
      SELECT user_id, mood, selected_memes, city, region, country,
             gender, sexual_orientation, interested_in, latitude, longitude,
             max_distance_preference, age
      FROM compatible_users
      ORDER BY calculated_distance ASC, location_score DESC, mood_score DESC, shared_memes DESC NULLS LAST, RANDOM()
